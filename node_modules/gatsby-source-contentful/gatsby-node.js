...
exports.setFieldsOnGraphQLNodeType = require(`./extend-node-type`).extendNodeType; // TODO: Remove once pluginOptionsSchema is stable

exports.onPreInit = ({
  reporter
}, options) => {
  const result = pluginOptionsSchema({
    Joi
  }).validate(options, {
    abortEarly: false,
    externals: false
...
const validateContentfulAccess = async pluginOptions => {
  ...
  await fetch(`https://${pluginOptions.host}/spaces/${pluginOptions.spaceId}`, {
    ...
    if (!ok) throw new Error(`Cannot access Contentful space "${maskText(pluginOptions.spaceId)}" with access token "${maskText(pluginOptions.accessToken)}". Make sure to double check them!`);
  });
  return undefined;
};
...
List of locales and their codes can be found in Contentful app -> Settings -> Locales`).default(() => true),
  ...
  richText: Joi.object().keys({
    resolveFieldLocales: Joi.boolean().description(`If you want to resolve the locales in fields of assets and entries that are referenced by rich text (e.g., via embedded entries or entry hyperlinks), set this to \`true\`. Otherwise, fields of referenced assets or entries will be objects keyed by locale.`).default(false)
  }).default({})
